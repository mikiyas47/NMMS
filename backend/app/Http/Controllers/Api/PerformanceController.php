...
{
    private function distId(Request $r): int { $u=$r->user(); return (int)($u->distributor_id??$u->id); }
    ...
    public function trackPresentation(Request $r, $token) {
        ...
        if ($watchPct > $assignment->watch_percent) $assignment->watch_percent = $watchPct;
        if ($pageReached > $assignment->page_reached) $assignment->page_reached = $pageReached;
    ...
    public function trackInvitation(Request $r, $token) {
        ...
        if ($action === 'open' && !$inv->opened_at) { $inv->status='opened'; $inv->opened_at=now(); }
        if ($action === 'accept') {
            $inv->status='accepted'; $inv->responded_at=now();
            // Auto-advance stage
            try {
                $prospect = Prospect::find($inv->prospect_id);
                if ($prospect && in_array($prospect->stage,['New Lead','Contacted'])) {
                    $prospect->stage='Invited'; $prospect->status='Invited'; $prospect->save();
            ...
            } catch (\Throwable $e) { Log::error('Invitation stage advance failed: '.$e->getMessage()); }
            $this->fireAutomation($inv->distributor_id,$inv->prospect_id,'invitation_status_changed',['status'=>'accepted']);
        }
        if ($action === 'decline') { $inv->status='declined'; $inv->responded_at=now(); }
        $inv->save();
    ...
    public function fireAutomation(int $distId, int $prospectId, string $triggerType, array $context=[]): void {
        ...
        foreach ($rules as $rule) {
            try {
                ...
                } catch (\Throwable $le) { /* logging failed, stop processing */ break; }
            }
        }
    }
    ...
    public function executeAutomationAction(AutomationRule $rule, int $prospectId, int $distId, string $trigger, array $ctx): void {
        ...
        switch ($rule->action_type) {
            ...
            case 'advance_stage':
                ...
                $prospect->stage=$newStage; $prospect->status=$newStage; $prospect->save(); break;
            case 'add_tag':
                $tags = $prospect->tags ?? [];
                $tag = $cfg['tag'] ?? 'auto-tagged';
                if (!in_array($tag,$tags)) { $tags[]=$tag; $prospect->tags=$tags; $prospect->save(); } break;
            case 'send_push_notification':
                // Push notification placeholder — integrate with FCM when tokens are available
                Log::info("Push notification for distributor {$distId}: ".($cfg['message']??'Automation triggered')); break;
        }
    ...
    public function recomputePriority(int $prospectId, int $distId): void {
        try {
            ...
            if (abs($score - ($p->interest_score??0)) > 5) {
                $p->interest_score = max(0,$score); $p->save();
            }
        } catch (\Throwable $e) { Log::error('Priority recompute failed: '.$e->getMessage()); }
    }
    ...
    private function generateRecommendation(Prospect $p, int $score, int $distId): string {
        ...
        if ($score >= 80 && $hoursSince <= 24) return 'Highly engaged. Call now.';
        if ($score >= 60 && $daysSince >= 2)   return 'High intent. Follow up today.';
        if ($score < 30 && $ignoredFollowups >= 3) return 'Pause outreach. Re-engage in 7 days.';
        if ($score < 20 && $daysSince > 14)    return 'Low engagement. Consider removing from active pipeline.';
    ...
    public function dailyDashboard(Request $r) {
        ...
        $today  = Carbon::today();
        // Follow-up tasks
        $followupTasks = Prospect::where('distributor_id',$distId)->whereNotIn('stage',['Joined','Rejected','Inactive'])->where(function($q) use($today){ $q->whereDate('next_action_date','<=',$today)->orWhereDate('next_action_date',$today); })->whereNotNull('next_action_date')->orderByDesc('next_action_date')->limit(5)->get()->map(fn($p)=>['type'=>'followup','prospect_id'=>$p->prospect_id,'prospect_name'=>$p->name,'action'=>$p->next_action??'Follow up','due_date'=>$p->next_action_date,'priority_score'=>$p->interest_score??0]);
        ...
        $fCount = count($followupTasks); $iCount = count($inviteTasks); $pCount = count($presentTasks); $cCount = count($closingTasks);
        $parts = [];
        if ($fCount>0) $parts[] = "Follow up {$fCount} hot lead".($fCount>1?'s':'');
        if ($iCount>0) $parts[] = "Invite {$iCount} new ".($iCount>1?'people':'person');
        if ($pCount>0) $parts[] = "Send {$pCount} presentation".($pCount>1?'s':'');
        if ($cCount>0) $parts[] = "Close {$cCount} prospect".($cCount>1?'s':'');
    ...
    public function completeTask(Request $r) {
        ...
        foreach ([3=>'streak_3',7=>'streak_7',30=>'streak_30'] as $days=>$badge) {
            if ($streak->current_streak >= $days) $this->checkBadge($distId,$badge);
        }
    ...
    public function runBehavioralAnalysis(int $distId): void {
        ...
        foreach ($prospects as $p) {
            ...
            if ($highWatchAssignment) $this->createRecommendation($distId,$pid,'high_intent','high_watch','Presentation watched 80%+. Call now to close.');
            $recentAccept = \App\Models\Invitation::where('prospect_id',$pid)->where('status','accepted')->where('responded_at','>=',now()->subHours(2))->exists();
            if ($recentAccept) $this->createRecommendation($distId,$pid,'high_intent','quick_accept','Invitation accepted within 2 hours. Strike while hot!');
            $pageVisits = EngagementEvent::where('prospect_id',$pid)->where('event_type','page_visit')->where('created_at','>=',now()->subHours(24))->count();
            if ($pageVisits >= 3) $this->createRecommendation($distId,$pid,'high_intent','multiple_page_visits',"Visited your page {$pageVisits} times today. High buying intent.");
            ...
            if ($ignoredFollowups >= 3) $this->createRecommendation($distId,$pid,'disengagement','ignored_followups','3+ follow-ups ignored. Consider pausing outreach for 7 days.');
            $openedNotAccepted = \App\Models\Invitation::where('prospect_id',$pid)->where('status','opened')->where('opened_at','<=',now()->subHours(48))->exists();
            if ($openedNotAccepted) $this->createRecommendation($distId,$pid,'disengagement','opened_not_accepted','Invitation opened but not accepted in 48h. Send a gentle follow-up.');
            ...
            if ($notOpenedPresentation) $this->createRecommendation($distId,$pid,'disengagement','presentation_not_opened','Presentation not opened in 72h. Try a different approach.');
        }
    }
        ...
        if (!$progress->$milestone) {
            ...
            if ($progress->first_invite_sent && $progress->first_presentation_assigned && $progress->first_prospect_added && $progress->first_recruit_joined && $progress->checklist_completed) {
                if (!$progress->onboarding_complete) {
                    ...
                    if ($dist && $dist->upline_id) Log::info("Sponsor {$dist->upline_id} notified: distributor {$distId} completed onboarding.");
                }
            }
        }
    }
    ...
    public function checkFirstTenChallenge(int $distId): void {
        $progress = OnboardingProgress::firstOrCreate(['distributor_id'=>$distId]);
        if ($progress->first_10_challenge_complete) return;
        ...
        if ($count >= 10) {
            ...
            $dist = Distributor::where('distributor_id',$distId)->first();
            if ($dist && $dist->upline_id) Log::info("Sponsor {$dist->upline_id} notified: distributor {$distId} completed First 10 Challenge.");
        }
    }
    ...
    public function weeklyGoals(Request $r) {
        ...
        if ($overall >= 100 && !$goal->goal_achieved) {
            $goal->goal_achieved = true; $goal->save();
    ...
    public function funnelReport(Request $r) {
        ...
        if ($dateFrom) { $pq = fn() => Prospect::where('distributor_id',$distId)->whereDate('created_at','>=',$dateFrom); }
        if ($dateTo)   { $pq = fn() => Prospect::where('distributor_id',$distId)->whereDate('created_at','<=',$dateTo); }
    ...
    public function publicInvitePage(Request $r, $token) {
        ...
        if (!$inv) return response()->json(['message'=>'This link is no longer active.'],404);
        $distributor = Distributor::where('distributor_id',$inv->distributor_id)->first();
        $prospect    = Prospect::find($inv->prospect_id);
    ...
    public function publicPresentationPage(Request $r, $token) {
        ...
        if (!$assignment) return response()->json(['message'=>'This link is no longer active.'],404);
    ...
    public function capturePublicLead(Request $r, $token) {
        ...
        if (!$source) return response()->json(['message'=>'Invalid link'],404);
