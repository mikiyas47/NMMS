...
class ProspectController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        try {
            ...
            $presentationsToday = $all->filter(function ($p) use ($today) {
                if ($p->stage !== 'Presentation Scheduled' || !$p->next_action_date)
                    return false;
                try {
                    return Carbon::parse($p->next_action_date)->isSameDay($today);
                } catch (\Exception $e) {
            ...
            $newlyJoined = $all->filter(function ($p) {
                if ($p->stage !== 'Joined' || !$p->joined_at)
                    return false;
                try {
                    return Carbon::parse($p->joined_at)->gte(Carbon::now()->subDays(7));
                } catch (\Exception $e) {
                    return false;
                }
            })->values();
    ...
    public function index(Request $request)
    {
        ...
        if ($level)
            $query->where('interest_level', $level);
        if ($search) {
    ...
    public function moveStage(Request $request, $id)
    {
        ...
        if (isset($data['next_action_date']))
        ...
        if ($newStage === 'Joined') {
            $prospect->interest_score = 100;
        } elseif ($newStage === 'Rejected') {
            $prospect->interest_score = max(0, ($prospect->interest_score ?? 0) - 20);
        }
    ...
    public function storeFollowup(Request $request, $id)
    {
        ...
        if (in_array($data['outcome'] ?? '', ['Interested', 'Replied with interest', 'Positive'])) {
            $prospect->interest_score = min(100, ($prospect->interest_score ?? 0) + 15);
        }
        if (in_array($data['outcome'] ?? '', ['Not interested', 'Negative'])) {
            $prospect->interest_score = max(0, ($prospect->interest_score ?? 0) - 10);
        }
    ...
    public function storeClosing(Request $request, $id)
    {
        ...
        if (in_array($data['outcome'] ?? '', ['Positive', 'Closed'])) {
            $prospect->interest_score = min(100, ($prospect->interest_score ?? 0) + 25);
        }
     ...
     * Returns the full score breakdown for a single prospect on demand.
     * Kept separate from the list endpoint to avoid computing it for every prospect.
     */
    public function scoreBreakdown(Request $request, $id)
    {
        $distId = $this->distId($request);
        $prospect = Prospect::where('prospect_id', $id)
            ->where('distributor_id', $distId)
        ...
        try {
            $breakdown = app(\App\Services\Scoring\InterestScoreService::class)->calculate($prospect);
            return response()->json(['status' => 'success', 'data' => $breakdown]);
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::error('Score breakdown error: ' . $e->getMessage());
            return response()->json(['status' => 'error', 'message' => 'Could not compute score breakdown.'], 500);
        }
    }
}
